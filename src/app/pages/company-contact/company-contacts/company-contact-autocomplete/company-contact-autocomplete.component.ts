...
export const EPANDED_TEXTAREA_VALUE_ACCESSOR: any = {
    ...
    useExisting: forwardRef(() => CompanyContactAutocompleteComponent),
    multi: true,
};

@Component({
    selector: 'app-company-contact-autocomplete',
    templateUrl: './company-contact-autocomplete.component.html',
    styleUrls: ['./company-contact-autocomplete.component.scss'],
    providers: [EPANDED_TEXTAREA_VALUE_ACCESSOR],
})
export class CompanyContactAutocompleteComponent implements ControlValueAccessor, OnInit {
    @Input() model: any;
    @Input() valid: boolean;
    ...
    public loadLazy(event?) {
        ...
        if (event.query) {
            this.filters.push({ key: 'filter{email.icontains}', value: event.query })
        } else {
    ...
    getModels() {
        this.modelsService.get(this.page, this.per_page, this.sort, this.query, this.filters, this._with).subscribe(
            response => {
                this.models = response.company_contacts;
