...
export const EPANDED_TEXTAREA_VALUE_ACCESSOR: any = {
    ...
    useExisting: forwardRef(() => QuoteTemplateAutocompleteComponent),
    multi: true,
};

@Component({
    selector: 'app-quote-template-autocomplete',
    templateUrl: './quote-template-autocomplete.component.html',
    styleUrls: ['./quote-template-autocomplete.component.scss'],
    providers: [EPANDED_TEXTAREA_VALUE_ACCESSOR],
})
export class QuoteTemplateAutocompleteComponent implements ControlValueAccessor, OnInit {
    @Input() model: any;
    @Input() valid: boolean;
    ...
    public ngOnInit() {
        if (!this.placeholder) {
            this.placeholder = 'Quote Template';
        }
    }
    ...
    getModels() {
        this.modelsService.get(this.page, this.per_page, this.sort, this.query, this.filters).toPromise().then(
            response => {
                this.models = response.quote_templates;
