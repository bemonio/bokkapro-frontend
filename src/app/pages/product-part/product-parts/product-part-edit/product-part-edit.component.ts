...
import { ProductPartService as ModelsService } from '../../_services/product-part.service';

@Component({
  selector: 'app-product-part-edit',
  templateUrl: './product-part-edit.component.html',
  styleUrls: ['./product-part-edit.component.scss']
})
export class ProductPartEditComponent implements OnInit, OnDestroy {
  public id: number;
  public model: Model;
  ...
  get() {
    ...
    const sb = this.route.paramMap.pipe(
      switchMap(params => {
        ...
        return of({ 'product_part': new Model() });
      }),
      catchError((error) => {
        this.requesting = false;
        let messageError = [];
      ...
      this.requesting = false;
      if (response) {
        this.model = response.product_part;
  ...
  edit() {
    ...
    const sbUpdate = this.modelsService.patch(this.id, model).pipe(
      tap(() => {
        ...
        if (this.saveAndExit) {
          this.router.navigate(['/productparts']);
        }
      }),
      catchError((error) => {
        this.requesting = false;
  ...
  create() {
    ...
    const sbCreate = this.modelsService.post(model).pipe(
      ...
      this.requesting = false;
      this.model = response.product_part as Model
    });
