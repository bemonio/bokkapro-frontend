...
export const EPANDED_TEXTAREA_VALUE_ACCESSOR: any = {
  ...
  useExisting: forwardRef(() => PositionAutocompleteComponent),
  multi: true,
};

@Component({
  selector: 'app-position-autocomplete',
  templateUrl: './position-autocomplete.component.html',
  styleUrls: ['./position-autocomplete.component.scss'],
  providers: [EPANDED_TEXTAREA_VALUE_ACCESSOR],
})
export class PositionAutocompleteComponent implements ControlValueAccessor, OnInit {
    @Input() model: any;
    @Input() valid: boolean;
    ...
    public ngOnInit() {
        if (!this.placeholder) {
            this.placeholder = 'Segment Position';
        }
    }
    ...
    getModels() {
        this.modelsService.get(this.page, this.per_page, this.sort, this.query, this.filters).toPromise().then(
            response => {
                this.models = response.positions;
