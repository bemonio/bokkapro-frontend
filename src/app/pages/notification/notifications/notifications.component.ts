...
import { NotificationService as ModelService } from '../_services/notification.service';
    ...
    public getModels() {
        ...
        this.modelsService.get(this.page, this.per_page, this.sort, this.query, this.filters, this._with).subscribe(
            response => {
                ...
                if(response.employee_origins){
                    response.employee_origins.forEach(employee_origin => {
                        this.models.forEach(element => {
                            if (element.employee_origin === employee_origin.id) {
                                element.employee_origin = employee_origin;
                            }
                        });
                    });
                }
                if(response.employee_destinations){
                    response.employee_destinations.forEach(employee_destination => {
                        this.models.forEach(element => {
                            if (element.employee_destination === employee_destination.id) {
                                element.employee_destination = employee_destination;
                            }
                        });
                    });
                }
