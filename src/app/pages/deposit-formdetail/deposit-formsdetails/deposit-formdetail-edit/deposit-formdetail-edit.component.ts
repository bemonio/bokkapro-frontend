...
export class DepositFormDetailEditComponent implements OnInit, OnDestroy, OnChanges {
  @Input() redirectURL: boolean;
  @Input() depositFormID: number;
  @Input() createOrEdit: {create: boolean, id: number};
  @Output() createOrEditChange: EventEmitter<boolean> = new EventEmitter<boolean>();
  ...
  get() {
    ...
    const sb = this.route.paramMap.pipe(
      switchMap(params => {
        // get id from URL
        if (this.createOrEdit.id === undefined) {
          this.id = Number(params.get('id'));
        } else {
          this.id = this.createOrEdit.id;
        }
  ...
  loadForm() {
    if (this.model.id) {
      ...
      //   this.getDepositFormById(this.depositFormId);
      // }
