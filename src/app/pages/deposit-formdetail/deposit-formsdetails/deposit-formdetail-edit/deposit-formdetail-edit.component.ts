...
export class DepositFormDetailEditComponent implements OnInit, OnDestroy {
  ...
  get() {
    ...
    const sb = this.route.paramMap.pipe(
      switchMap(params => {
        ...
        return of({ 'deposit_formdetail': new Model() });
      }),
      catchError((error) => {
        this.requesting = false;
        let messageError = [];
      ...
      this.requesting = false;
      if (response) {
        this.model = response.deposit_formdetail;
        if (response.deposit_form) {
          this.model.deposit_form = response.deposit_form[0];
        }
        if (response.currency_detail) {
          this.model.currency_detail = response.currency_detail[0];
        }
  ...
  edit() {
    ...
    const sbUpdate = this.modelsService.patch(this.id, model).pipe(
      tap(() => {
        ...
        if (this.saveAndExit) {
          this.router.navigate(['/depositformsdetails']);
        }
      }),
      catchError((error) => {
        this.requesting = false;
  ...
  create() {
    this.requesting = true;
    let model = this.model;
    model.deposit_form = this.model.id
    model.currency_detail = this.model.id
    const sbCreate = this.modelsService.post(model).pipe(
      ...
      this.requesting = false;
      this.model = response.deposit_formdetail as Model
    });
