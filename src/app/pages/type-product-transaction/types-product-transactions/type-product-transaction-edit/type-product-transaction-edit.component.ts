...
import { TypeProductTransactionModel as Model } from '../../_models/type-product-transaction.model';
import { TypeProductTransactionService as ModelsService } from '../../_services/type-product-transaction.service';

@Component({
  selector: 'app-type-product-transaction-edit',
  templateUrl: './type-product-transaction-edit.component.html',
  styleUrls: ['./type-product-transaction-edit.component.scss']
})
export class TypeProductTransactionEditComponent implements OnInit, OnDestroy {
  public id: number;
  public model: Model;
  ...
  edit() {
    ...
    const sbUpdate = this.modelsService.patch(this.id, this.model).pipe(
      tap(() => {
        ...
        if (this.saveAndExit) {
          this.router.navigate(['/typesproducttransactions']);
        }
      }),
      catchError((error) => {
        this.requesting = false;
